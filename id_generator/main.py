...
def generate_ids(network: Network, blacklist=None):
    ...
    if len(network.get_messages()) >= 2**MESSAGE_BITS:
        raise Exception(
            "Oops, you can't have more than {0} messages per topic!, maybe its time to rework the software?".format(
                MESSAGE_BITS
            )
        )
    ...
    for topic_name, topic_id in generate_topics_id(network).items():
        ...
        msg_ids, scoped_msg_ids = generate_messages_id(
            network.get_messages_by_topic(topic_name), topic_id, blacklist
        )

        if __debug__:
            for (g_name, g_contents), (s_name, s_contents) in zip(
                msg_ids.items(), scoped_msg_ids.items()
            ):
                print(
                    f"{s_name:<16}\tbin  int\n\t"
        ...
        ids[topic_name] = {"id": topic_id, "messages": msg_ids}
...
def generate_messages_id(topic_messages, topic_id, blacklist=None):
    ...
    for message_name, message_contents in topic_messages.items():
        ...
        if message_priority > MAX_PRIORITY:
            raise Exception(
                f"Priority assigned to {message_name} is outside of the allowed range (0-{MAX_PRIORITY})".format(
                    MESSAGE_BITS
                )
            )
        ...
        while True:
            ...
            if item_id >= MESSAGES_PER_PRIORITY:
                raise Exception(
                    f"You exceeded the maximum messages per priority level per topic! ({MESSAGES_PER_PRIORITY})".format(
                        MESSAGE_BITS
                    )
                )
...
def generate_topics_id(
    network: Network, blacklist=None
):  # TODO: implement blacklist for topics
    ids = {}
    for i, t in enumerate(network.get_topics()):
        ids[t] = i
    if len(ids) >= 2**TOPIC_BITS:
        raise Exception(
            "Oops, you can't have more than {0} topics!, maybe its time to rework the software?".format(
                TOPIC_BITS
            )
        )
...
def generate_fixed_ids(network: Network):
    ...
    for message_name, message_contents in network.get_messages_with_fixed_id().items():
        ids[message_name] = {"id": message_contents["fixed_id"]}
    return ids
...
def main():
    ...
    print(
        "Max messages per priority per topic: {0}".format(
            int(2**MESSAGE_BITS / (MAX_PRIORITY + 1))
        )
    )
    print("")
    ...
    for n in networks:
        ...
        output = {"network_version": n.version, "topics": ids}
