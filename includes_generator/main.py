...
def generate_canconfig_includes(
    canconfig, canconfig_version, network_name, output_path
):
    cgenerated = c_gen.generate_canconfig_include(
        canconfig, canconfig_version, network_name
    )
...
def read_args(argv):
    ...
    if len(argv) != 4 or argv[1] in ["--help", "-h"]:
        raise ValueError(
            "Usage: python3 main.py <networks_path> <ids_path> <output_path>"
        )
...
def main():
    ...
    networks = utils.load_networks(
        networks_dir,
        c.NETWORK_VALIDATION_SCHEMA,
        ids_dir,
        c.NETWORK_IDS_VALIDATION_SCHEMA,
    )
    ...
    for n in networks:
        ...
        if canconfig_path.exists():
            canconfig_file = utils.load_json(
                canconfig_path, c.CANCONFIG_VALIDATION_SCHEMA
            )
            canconfig = canconfig_file["canconfig"]
            canconfig_version = float(canconfig_file["canconfig_version"])

            generate_canconfig_includes(
                canconfig, canconfig_version, n.name, output_dir_network
            )
